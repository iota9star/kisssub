...
class TagsPresenter(view: StringContract.View<ArrayList<String>>) : StringPresenter<ArrayList<String>>(view) {
    override fun deal(resp: Response<String>): ArrayList<String> {
        val items = Jsoup.parse(resp.body())?.select("div.main div.box table tr td > span.tag > a")
